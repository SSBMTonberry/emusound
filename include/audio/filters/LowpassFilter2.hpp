...
namespace esnd
{
    class LowpassFilter2 : public ISoundFilter
    {
        public:
            ...
            LowpassFilter2(ma_decoder *decoder, const std::string &id, double cutoffFrequency, double q, uint32_t channels = 2, uint32_t sampleRate = 44100)
            {
            ...
            int init(ma_decoder *decoder, const std::string &id, double cutoffFrequency, double q, uint32_t channels = 2, uint32_t sampleRate = 44100)
            {
                m_decoder = decoder;
                m_id = id;
                config = ma_lpf2_config_init(m_decoder->outputFormat, channels, sampleRate, cutoffFrequency, q);
