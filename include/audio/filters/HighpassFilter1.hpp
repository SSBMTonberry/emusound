...
namespace esnd
{
    class HighpassFilter1 : public ISoundFilter
    {
        public:
            ...
            HighpassFilter1(const std::string &id, double cutoffFrequency, uint32_t channels = 2, uint32_t sampleRate = 44100)
            {
            ...
            {
                m_id = id;
                config = ma_hpf1_config_init(ma_format_s16, channels, sampleRate, cutoffFrequency);
